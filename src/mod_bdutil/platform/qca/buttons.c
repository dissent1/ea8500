...
typedef irqreturn_t (*irq_func_t) (int, void *);
...
static Timer rst_btnwps_timer;
//static int rst_timer = 0;
static int rst_btn_check_freq = 4;        /* 2 sec. default*/
static int rst_btnwps_check_freq = 1;     /* 100 ms, default*/
...
static void rst_btn_counter(unsigned long var)
{
	int value;
...
static void rst_btnwps_counter(unsigned long var)
{
	int value;

	DBGX(1, "%s: check interval %d\n", __FUNCTION__, rst_btn_check_freq);

	value = gpio_get_value(BOARD_GPIO_WPS_PIN);
	if (!value) {
		DBGX(1, "%s: (%d) Reset button pressed!\n", __FUNCTION__, value);
		buttons_irq_handler(BOARD_GPIO_WPS_PIN, NULL);
		rst_btn_irq_handled = 1;
	} else {
		rst_btn_irq_handled = 0;
		DBGX(1, "%s: (%d) Reset button not detected!\n", __FUNCTION__, value);
	...
	rst_btnwps_timer.expires = jiffies + (rst_btnwps_check_freq  * (HZ) / 10);//100ms
	add_timer(&rst_btnwps_timer);
}

static void free_rst_btn_resources(void)
{
        del_timer_sync(&rst_btn_timer);
		del_timer_sync(&rst_btnwps_timer);
}
...
static void rst_btn_set_timer(void)
{
	...
	init_timer(&rst_btnwps_timer);
	rst_btnwps_timer.function = rst_btnwps_counter;
	rst_btnwps_timer.data = (unsigned int)NULL;
	rst_btnwps_timer.expires = jiffies + (rst_btnwps_check_freq  * (HZ) / 10);//100ms
...
static irqreturn_t buttons_irq_handler(int irq, void *dev)
{
	...
	if (rst_btn_irq_handled == 1) return IRQ_HANDLED;
...
static int buttons_open(struct inode *inode, struct file *filp)
{
	...
	DBGX(1, "%s: entering\n", __FUNCTION__); 
	if (mod_bdutil_opened) return -ENOMEM;
...
int init_buttons(void)
{
	...
	button_irqs[1].assigned_irq = BOARD_GPIO_WPS_PIN;
